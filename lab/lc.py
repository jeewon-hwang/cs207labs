...
class LightCurve:
    ...
    def __init__(self, data, metadict):
        ...
        self.components = components[components.find('['):]
    ...
    @property
    def timeseries(self):
        return zip(self._time, self.amplitude)
...
def lc_reader(filename):
    ...
    with open(filename) as fp:
        for line in fp:
            ...
            elif line.find('#')==0 and sharp_counter==0:
                sharp_counter +=1
                facetnames = line[1:].strip().split()
            elif line.find('#')==0 and sharp_counter==1:
                sharp_counter +=1
                facetvalues = line[1:].strip().split()
                lcdict = dict(zip(facetnames, facetvalues))
